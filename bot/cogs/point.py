...
class GamesCog(commands.Cog):
    ...
    async def genshinwish(
        self,
        ctx: discord.ApplicationContext,
        skip: Option(int, choices=Skip_list, required=True,
                     description="流れ星演出のスキップをするかどうか（書かない場合はスキップしない）", default=0)
    ):
        ...
        if resalt < 9:
            ...
            for num in range(9):
                tmpresalt = np.random.choice(["3", "4", "5", "6"], p=[
            ...
            for num in range(9):
                tmpresalt = np.random.choice(
                    ["3", "4", "6"], p=[three, 0.051, per])
                randomresalt.append(tmpresalt.tolist())
                print(randomresalt)
        ...
        for r in randomresalt:
            if r == "4":
                ...
                sterresalt = np.random.choice(
                    ["four_1", "four_2"], p=[0.5, 0.5])
                genshinname = GamesCog.genshinster(
                    "".join(sterresalt.tolist()))
